...
#SBATCH --output=/scratch/gpfs/mengzhou/space10/output/logs/train_grad-%j.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
for step in {33..105}; do
    ...
    output_path=${model_dir}/train_${type}_grad/step${step} # path to output
...
cd /scratch/gpfs/mengzhou/space10/final/DIGSIT
for type in sgd; do
sbatch -p cli run/first_order_checking/calculate_train_grad.sh $type
done
"""
